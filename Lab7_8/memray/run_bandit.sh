...
git log --no-merges --pretty=format:"%H" -n 300 > commits.txt
...
while read commit; do
    ...
    changed_files=$(git diff-tree --no-commit-id --name-only -r "$commit" | grep '\.py$' | grep -v 'memray_env/' )
    ...
    if [[ -n "$changed_files" ]]; then  # If Python files were modified
        ...
        bandit -r $(echo "$changed_files") -x memray_env -f json -o "bandit_reports/report_$commit.json"
