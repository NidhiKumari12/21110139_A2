...
while read commit; do
    ...
    changed_files=$(git diff-tree --no-commit-id --name-only -r "$commit" | grep '\.py$' | grep -v 'flash_env/' )
    ...
    if [[ -n "$changed_files" ]]; then  # If Python files were modified
        ...
        bandit -r $(echo "$changed_files") -x flash_env -f json -o "bandit_reports/report_$commit.json"
